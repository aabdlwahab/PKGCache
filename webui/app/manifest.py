...
and own the canonical eco→(subdir, ecosystem) mapping the rest of the backend reads.

The proxies (pkgcache) record every cached artifact into caches/<eco>/ledger.db at
commit time, so there's no filesystem walking — this reads each ledger and emits a
deterministic, git-diffable subset into caches/manifests/<eco>.json. Stdlib-only
(sqlite3), so it runs even while a checkpoint has the proxies stopped.

This module used to live at scripts/gen_manifest.py; that path is now a thin wrapper
that imports and calls main() here, so the ECOS map has ONE definition. The
checkpoint op still invokes the wrapper as a subprocess (with PKGCACHE_MANIFEST_ROOT
pointing at the project's repo), so CACHES/MANIFESTS are resolved from the env at
import in that fresh process."""
import json
import os
import pathlib
import sqlite3
...
# eco -> (cache subdir holding ledger.db, ecosystem value to filter on). apt + apk
# share the apt subdir/ledger; this is the single source of truth for that mapping.
ECOS = {
